...
public class CloudDiskController extends BaseController {
	...
	String menuUrl = "clouddisk/list.do"; // 菜单地址(权限用)
	...
	public ModelAndView goView() throws Exception {
		...
		if ("1".equals(pd.getString("TYPE")) && "2".equals(pd.getString("STATUS"))) { // 在收信箱里面查看未读的站内信时去数据库改变未读状态为已读
			cloudDiskService.edit(pd);
		}
		pd = cloudDiskService.findById(pd); // 根据ID读取
	...
	public Object deleteAll() throws Exception {
		...
		if (null != DATA_IDS && !"".equals(DATA_IDS)) {
			...
			cloudDiskService.deleteAll(ArrayDATA_IDS);
			pd.put("msg", "ok");
		} else {
			pd.put("msg", "no");
		}
