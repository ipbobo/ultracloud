			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="mirror/listVmware.do" method="post" name="Form" id="Form">
						<table style="margin-top:5px;width:100%">
							<tr>
								<td style="vertical-align:top;">
									<c:if test="${QX.add == 1 }">
									<a class="btn btn-sm btn-success" onclick="synctemplate();">同步镜像模板</a>
										...
										<tr>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<a class="btn btn-xs btn-info" title="绑定模板" onclick="goBindingtemplate('${var.id}','${var.ostype}','${var.bitrate}');">
	...
	<script type="text/javascript">
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>mirror/delete.do?id="+Id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>mirror/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//打开绑定用户窗口
		function goBindingtemplate(id,ostype,bitrate){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="绑定模板";
		...
		//确认同步镜像模板
		function synctemplate(id) {
			bootbox.confirm("确定开始同步vmware的镜像模板吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>mirror/synctemplate.do?id="+id;
					$.get(url,function(data){
						 var diag = new top.Dialog();
						 diag.Drag=true;
						 diag.Title ="同步的镜像模板";
