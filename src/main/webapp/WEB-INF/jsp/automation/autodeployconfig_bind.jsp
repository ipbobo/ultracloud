...
<link rel="stylesheet" href="plugins/dragula/dist/dragula.css">
...
<body class="no-skin">
	<div class="main-container" id="main-container">
		<form action="autodeployconfig/bind.do" name="form1" id="form1"
			method="post">
			<input type="hidden" name="id" id="id" value="${pd.id}" /> <input
				type="hidden" name="selectedNodeIds" id="selectedNodeIds" /> <input
				type="hidden" name="notBindingNodeList" id="notBindingNodeList"
				value="${notBindingNodeList}" /> <input type="hidden"
				name="bindedNodeList" id="bindedNodeList" value="${bindedNodeList}" />
			<div id="zhongxin" style="padding-top: 13px;"/>
				<div class='parent'>
					<div class='wrapper'>
						<div id='left-events' class='container'></div>
						<div id='right-events' class='container'></div>
					</div>
					<br />
				</div>
	...
	<script src='plugins/dragula/dist/dragula.js'></script>
	...
	<script type="text/javascript">
		...
		function genNodeListDiv(nodeList) {
			...
			if (nodeList.length > 2) {
				var nodeArray = nodeList.substring(2, nodeList.length - 2)
						.split("}, {");
				for (i = 0; i < nodeArray.length; i++) {
					...
					nodeText += "<div style='width:280px' id='"+id+"'>" + name + "</div>";
				}
			}
