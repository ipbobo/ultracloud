...
<body class="no-skin">
												...
												<c:if test="${QX.cha == 1 }">
													<c:forEach items="${varList}" var="var" varStatus="vs">
														<tr>
															...
															<td class='center' style="width: 200px;"><img
																style="margin-top: -3px;" alt="${var.NAME}"
																src="static/images/extension/wenben.png">${var.url}
																<a style="cursor: pointer;"
																onclick="goViewTxt('${var.name}${fn:substring(var.url ,19,fn:length(var.url))}','${var.id}','gbk');">[预览]</a>
															</td>
															...
															<td class="center"><c:if
																...
																<div class="hidden-sm hidden-xs btn-group">
																	<a class="btn btn-xs btn-success" title="下载"
																		onclick="window.location.href='<%=basePath%>/script/download.do?id=${var.id}'">
																		<i class="ace-icon fa fa-cloud-download bigger-120"
																		title="下载"></i>
																	</a> <a class="btn btn-xs btn-success" title="添加参数"
																		onclick="listAllParam('${var.id}');"> <i
	...
	<script type="text/javascript">
		...
		//跳添加参数页
		function listAllParam(id){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="脚本参数列表";
			 diag.URL = '<%=basePath%>script/listAllParam.do?id='+id;
			 diag.Width = 600;
			 diag.Height = 500;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
