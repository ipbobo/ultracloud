...
<body class="no-skin">
				...
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
							...
							<!-- 检索  -->
							<form action="autodeployconfig/list.do" method="post" name="Form" id="Form">
														...
														<tr>
															...
															<td class="center">
																...
																<div class="hidden-sm hidden-xs btn-group">
																	<a class="btn btn-xs btn-success" title="配置自动部署流程"
																		onclick="goNodebind('${var.id}');"> <i
																		class="ace-icon fa fa-wrench bigger-120 icon-only"
																		title="配置自动部署流程"></i>
																	</a>
																	<c:if test="${QX.edit == 1 }">
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>autodeployconfig/goAdd.do';
			 diag.Width = 500;
			 diag.Height = 300;
		...
		//弹出配置自动部署流程页
		function goNodebind(id){
			var width=document.documentElement.clientWidth * 0.5+"px";
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>autodeployconfig/delete.do?id="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>autodeployconfig/goEdit.do?id='+Id;
			 diag.Width = 500;
			 diag.Height = 300;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>autodeployconfig/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
