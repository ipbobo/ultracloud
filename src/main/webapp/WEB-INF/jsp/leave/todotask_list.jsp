...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="leave/todotasklist.do" method="post" name="Form" id="Form">
							...
							<c:choose>
								<c:when test="${not empty varList}">
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${var.taskId}" class="ace" /><span class="lbl"></span></label>
											</td>
											<td class='center' style="width: 30px;">${vs.index+1}</td>
											<td class='center'>${var.taskId}</td>
											<td class='center'>${var.taskDefinitionKey}</td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<a class="btn btn-xs btn-success" title="处理" onclick="complete('${var.taskId}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="处理"></i>
													</a>
												</div>
											</td>
										</tr>
	...
	function complete(taskId) {
		bootbox.confirm("确定要处理吗?", function(result) {
			if(result) {
				top.jzts();
				var url = "<%=basePath%>leave/complete.do?taskId="+taskId;
